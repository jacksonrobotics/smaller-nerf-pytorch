...
class VeryTinyNeRFModel(torch.nn.Module):
    ...
    def __init__(self, filter_size=128, num_encoding_functions=6, Nbits = 6, symmetric=False, use_viewdirs=True):
    ...
    def forward(self, x):
        x = self.relu(self.layer1(x, STE.apply(self.linear1.weight, self.Nbits, self.symmetric), self.linear1.bias))
        x = self.relu(self.layer2(x, STE.apply(self.linear2.weight, self.Nbits, self.symmetric), self.linear2.bias)) 
        x = self.layer3(x, STE.apply(self.linear3.weight, self.Nbits, self.symmetric), self.linear3.bias)  
        return x
...
class STE(torch.autograd.Function):
    ...
    def forward(ctx, w, bit, symmetric=False):
        ...
        if bit is None:
            ...
            weight = w.weight.data.cpu().detach().numpy()
            weight_mask = np.where(weight == 0, weight, 1)
            
            # Lab3 (a), Your code here:
            if symmetric == False:
                # Compute alpha (scale) for dynamic scaling

                alpha = np.max(weight)-np.min(weight)
                # Compute beta (bias) for dynamic scaling
                beta = np.min(np.abs(weight))
                # Scale w with alpha and beta so that all elements in ws are between 0 and 1
                ws = (weight - beta)/alpha  # SCALED BETWEEN 0 AND 1
                
                step = 2 ** (bit)-1
                # Quantize ws with a linear quantizer to "bit" bits (Mapping Function)
                R = (1/(step-1))*np.round(step*ws)
                # Scale the quantized weight R back with alpha and beta
                wq = (R - beta)/alpha
            
            # Lab3 (e), Your code here:
            else:
                magnitude = np.max(np.abs(weight))
                ws = ...  # USE THE LARGEST MAGNITUDE TO BOUND IT (-LARGEST, LARGEST)

                wq = (R - np.min(np.abs(R)))/(np.max(R)-np.min(R)) * 2 + (-1)
...
class FlexibleNeRFModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            ...
            return self.fc_out(x, STE.apply(self.linear.weight, self.Nbits, self.symmetric), self.linear.bias)
